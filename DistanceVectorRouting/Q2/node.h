...
class Node {
 ...
 public:
  ...
  void updateTblEntry(Node *node){
    // cur Node from the interfaces and coressponding table entries
    string Ip;
    for (int i = 0; i < interfaces.size(); ++i) {
      if(interfaces[i].second == node) {
        // store the ip for later use
        Ip = interfaces[i].first.getConnectedIp();
        interfaces.erase(interfaces.begin() + i);
        break;
      }
    }

    // change routiing table for stored tempIp
    for (int i = 0; i < mytbl.tbl.size(); ++i) {
      if(mytbl.tbl[i].nexthop == Ip)
        mytbl.tbl[i].cost = 16;
    }
  }  
